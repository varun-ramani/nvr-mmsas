...
% distall = [];
num_workers = feature('NumCores') / 2;
...
%                     disp(floor(disttot * deld2));
    ...
    % discordLog(['CPU: finished parfor iteration. Minutes elapsed:' num2str(toc(timer) / 60)]);
    disp(['CPU: finished parfor iteration. Minutes elapsed:' num2str(toc(timer) / 60)]);
end

% discordLog(['CPU: Finished processing. Final minutes elapsed:' num2str(toc(timer) / 60)]);
disp(['CPU: done processing. Minutes elapsed:' num2str(toc(timer) / 60)]);
